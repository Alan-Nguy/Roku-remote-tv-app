...
export async function sendKeypress(ip: string, key: RokuKey): Promise<boolean> {
  try {
    ...
    console.warn(`[Roku] keypress ${key} to ${ip} failed:`, JSON.stringify(err), err);
    return false;
  }
}
...
export async function getDeviceInfo(ip: string): Promise<RokuDevice | null> {
  try {
    ...
    return {
      ip,
      name: extractXml(xml, 'user-device-name') || extractXml(xml, 'friendly-device-name') || 'Roku Device',
...
export async function scanForDevices(
      ...
      promises.push(
        probeDevice(ip)
          .then((device) => {
            ...
            if (device) {
              console.log(`[Roku] Found device: ${device.name} at ${device.ip}`);
              onFound(device);
            }
          })
          .catch((err) => {
  ...
  console.log(`[Roku] Scan complete. Probed ${scanned} IPs, ${errorCount} errors.`);
...
function parseApps(xml: string): RokuApp[] {
  const apps: RokuApp[] = [];
  const appRegex = /<app\s+id="([^"]*)"(?:\s+type="([^"]*)")?(?:\s+version="([^"]*)")?[^>]*>([^<]*)<\/app>/g;
